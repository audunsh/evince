...
@widgets.register
class DenseView(widgets.DOMWidget):
    """A template widget for Evince."""

    # Name of the widget view class in front-end
    _view_name = tl.Unicode('DenseView').tag(sync=True)

    # Name of the widget model class in front-end
    _model_name = tl.Unicode('DenseModel').tag(sync=True)
    ...
    tex = tl.Bytes().tag(sync=True)
    size = tl.List([]).tag(sync=True)
